...
<div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"><a href="{{route('category.food.index')}}">Food</a></li>
      </ol>
    </nav>
</div><!-- End Page Title -->
            ...
            <div class="card-body pt-3">
                <ul class="row">
                    <div class="search-bar">
                        <form class="search-form d-flex justify-content-center align-items-center" action="{{route('category.food.store')}}" method="post">
                            @csrf
                            <div class="col-5 mx-2">
                                <div class="form-floating">
                                    <input type="text" class="form-control" id="floatingName" placeholder="Enter New Food Category" title="Enter New Food Category" name="name">
                                    <label for="floatingName">Enter New Food Category</label>
                                </div>
                            </div>
                            <div>
                        ...
                        <div class="row d-flex justify-content-around align-items-center">
                            @foreach ($datas as $item)
                                <div style="width: 500px; height: 100px; background-color:rgba(192, 192, 192, 0.3); border-radius:5px;" class="m-2 p-5 text-center d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="d-flex">
                                        <form action="{{route('category.food.delete',$item->slug)}}" method="POST">
                                            @csrf
                                        ...
                                        <form action="{{route('food.index')}}" method="get">
                                            ...
                                            <button class="btn btn-outline-primary m-2" title="view post data"><i class="ri-eye-line mx-2"></i>{{$item->food->count()}}</button>
                                        </form>
                                    </div>
                                </div>
                            @endforeach
