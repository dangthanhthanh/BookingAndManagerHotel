...
@section("sidebar")
    @include("pos.component.side_bar_booking",['table'=>'Event'])
@endsection()
@section('button_nav')
    {!!$buttonNavContent!!}
@endsection
...
<section class="section">
    <div class="row" style="margin-bottom: 20px;">
        ...
        <div class="btn-group" role="group" aria-label="Basic mixed styles example">
            <a type="button" href="{{route('pos.event.index')}}" class="btn btn-outline-primary p-2">All</a>
            @foreach ($category as $item)
                <a type="button" href="{{route('pos.event.index',['category'=>$item->slug])}}" class="btn btn-outline-primary p-2">{{$item->name}}</a>
            @endforeach
        </div>
    </div>
    <div class="row">
...
@endsection
@section("javacript")
    @include("pos.component.css_js.pos_js.event")
@endsection