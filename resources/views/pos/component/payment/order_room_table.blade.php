...
<h5 class="card-title">Order Room
    <a href="{{route('pos.room.index')}}" class="btn btn-outline-primary"><i class="ri-add-circle-line" aria-hidden="true"></i></a>
</h5>
@empty(!$bookingData)
    <!-- Default Table -->
    ...
    <tbody>
        ...
        @php
            ...
            $totalBookingItem = $days_difference * $item['quantity'];
            $totalBooking += $totalBookingItem;
            $totalBalance += $totalBooking + 100;
        @endphp
        <tr>
            <td>
            <div class="btn-group">
