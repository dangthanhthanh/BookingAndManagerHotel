...
class VnPayController extends PaymentController
{
    public function initializationPaymentWithVnpay($method, $orderId, $totalBalance)
    {
        if (in_array($method, ['vnpay_atm', 'vnpay_creditcard'])) {
            try {
                DB::beginTransaction();
                    $payment = $this->create($orderId,1,2);//(orderId,status_unpaid,method_Vnpay);
                DB::commit();
    ...
    private function handleSuccessPayment($paymentId)
    {
        ...
        $this->create($orderId,3,2);//(orderId,status_success,method_vnpay);
    ...
    {
        $orderId = $this->getBySlug($paymentId)->order_id;
        $this->create($orderId,2,2);//(orderId,status_error,method_vnpay);
